...
public abstract class RESTRepository extends BaseRESTRepository {
  ...
  public String latest(@HeaderParam("Accept") String mediaType, @PathParam("subject") String subject) {
    return getRenderer(mediaType).renderSchemaEntry(exists(getSubject(truncateSubject(subject)).latest()), true);
  }
  ...
  {
    return getRenderer(mediaType).renderSchemaEntry(exists(getSubject(truncateSubject(subject)).lookupById(id)), false);
  }
